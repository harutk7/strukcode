...
if (!Script.preview) {
  const previous = await fetch("https://registry.npmjs.org/strukcode-ai/latest")
    .then((res) => {
  ...
  const log =
    await $`git log v${previous}..HEAD --oneline --format="%h %s" -- packages/strukcode packages/sdk packages/plugin`.text()
  ...
  const compare =
    await $`gh api "/repos/harutk7/strukcode/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
console.log("\n=== strukcode ===\n")
await import(`../packages/strukcode/script/publish.ts`)
