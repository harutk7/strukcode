...
import { $ } from "bun"
import { createStrukcode } from "@strukcode-ai/sdk"
...
export async function getLatestRelease() {
  return fetch("https://api.github.com/repos/harutk7/strukcode/releases/latest")
    .then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/harutk7/strukcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/strukcode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/strukcode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/strukcode/")) areas.add("core")
...
async function summarizeCommit(strukcode: Awaited<ReturnType<typeof createStrukcode>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await strukcode.client.session.create()
  const result = await strukcode.client.session
    .prompt({
      path: { id: session.data!.id },
      body: {
        model: { providerID: "strukcode", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], strukcode: Awaited<ReturnType<typeof createStrukcode>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(strukcode, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/harutk7/strukcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const strukcode = await createStrukcode({ port: 5044 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, strukcode)
    notes.push(...lines)
