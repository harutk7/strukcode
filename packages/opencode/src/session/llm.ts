...
export namespace LLM {
  ...
  export const OUTPUT_TOKEN_MAX = Flag.STRUKCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
  ...
  export async function stream(input: StreamInput) {
    ...
    return streamText({
      ...
      headers: {
        ...(input.model.providerID.startsWith("strukcode")
          ? {
              "x-strukcode-project": Instance.project.id,
              "x-strukcode-session": input.sessionID,
              "x-strukcode-request": input.user.id,
              "x-strukcode-client": Flag.STRUKCODE_CLIENT,
            }
          : undefined),
        ...input.model.headers,
      },
