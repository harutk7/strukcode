...
import type { StrukcodeClient, SessionMessageResponse } from "@strukcode-ai/sdk/v2"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk: _sdk }: { sdk: StrukcodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `strukcode auth login` in the terminal",
        name: "Login with strukcode",
        id: "strukcode-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const strukcodeProvider = providers.find((p) => p.id === "strukcode")
    if (strukcodeProvider) {
      if (strukcodeProvider.models["big-pickle"]) {
        return { providerID: "strukcode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(strukcodeProvider.models))
      if (best) {
        return {
