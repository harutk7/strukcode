...
const AGENT_USERNAME = "strukcode-agent[bot]"
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.strukcode.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/strukcode,/oc")
          .split(",")
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `strukcode/${type}-${hex}-${timestamp}`
        }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the strukcode infrastructure after your response",
