...
export namespace Provider {
  ...
  const state = Instance.state(async () => {
    ...
    // Add alias: map "strukcode" to "strukcode" for backwards compatibility
    if (database["strukcode"] && !database["strukcode"]) {
      database["strukcode"] = database["strukcode"]
    }
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      const filteredModels = Object.fromEntries(
        Object.entries(provider.info.models)
          ...
          .filter(
            ([, model]) =>
              ((!model.experimental && model.status !== "alpha") || Flag.STRUKCODE_ENABLE_EXPERIMENTAL_MODELS) &&
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (strukcodeProvider && strukcodeProvider.info.models["gpt-5-nano"]) {
