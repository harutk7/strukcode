...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run strukcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for strukcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/strukcode\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found strukcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`strukcode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting strukcode...")
        UI.println()

        // Launch strukcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const strukcodeArgs = sessionId ? ["-s", sessionId] : []
        const strukcodeProcess = spawn("strukcode", strukcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          strukcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`strukcode exited with code ${code}`))
          })
          strukcodeProcess.on("error", reject)
        })
      },
    })
  },
