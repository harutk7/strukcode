...
import { createStrukcodeClient, type StrukcodeClient } from "@strukcode-ai/sdk"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run strukcode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running strukcode server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: StrukcodeClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createStrukcodeClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.STRUKCODE_AUTO_SHARE || args.share)) {
        ...
        if (!shareResult.error) {
          UI.println(UI.Style.TEXT_INFO_BOLD + "~  https://strukcode.ai/s/" + sessionID.slice(-8))
        }
      }
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createStrukcodeClient({ baseUrl: `http://${server.hostname}:${server.port}` })
