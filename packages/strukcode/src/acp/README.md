...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for strukcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal strukcode sessions
...
strukcode acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via strukcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | strukcode acp
```
...
### Mapping to StrukCode

ACP sessions map cleanly to strukcode's internal session model:
