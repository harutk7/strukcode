...
import type { StrukcodeClient } from "@strukcode-ai/sdk"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk }: { sdk: StrukcodeClient }) {
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    return model ?? { providerID: "strukcode", modelID: "big-pickle" }
  }
