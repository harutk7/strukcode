...
import { createStrukcode, type ToolPart } from "@strukcode-ai/sdk"
...
console.log("🚀 Starting strukcode server...")
const strukcode = await createStrukcode({
  port: 0,
})
console.log("✅ Strukcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await strukcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new strukcode session...")
    const { client, server } = strukcode
    ...
    console.log("✅ Created strukcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to strukcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Strukcode response:", JSON.stringify(result, null, 2))
