...
def test_integration_live_server_endpoints() -> None:
    ...
    # Start strukcode headless server on a random port
    pkg_strukcode = repo_root / "packages" / "strukcode"
    cmd = [
        "bun",
        "run",
    ...
    while time.time() - start < 15:
        ...
        if not line:
            ...
            if proc.poll() is not None:
                break
            continue
        m = re.search(r"strukcode server listening on (http://[^\s]+)", line)
        if m:
            url = m.group(1)
            break
    ...
    try:
        client = StrukCodeClient(base_url=url)
