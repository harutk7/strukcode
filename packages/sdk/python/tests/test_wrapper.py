...
from strukcode_ai.api.default import config_get
from strukcode_ai.client import Client
...
def test_get_path_with_mock_transport() -> None:
    ...
    w = StrukCodeClient(base_url="http://test")
...
def test_retry_on_request_error_then_success() -> None:
    ...
    w = StrukCodeClient(base_url="http://test", retries=1, backoff_factor=0)
    client = httpx.Client(base_url="http://test", transport=transport)
    w.client.set_httpx_client(client)
