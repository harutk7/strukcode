#!/usr/bin/env python3
"""
Generate the Opencode Python SDK using openapi-python-client and place it under src/strukcode_ai.
...
- Copy the generated module into src/strukcode_ai
...
def main() -> int:
    ...
    strukcode_dir = repo_root / "packages" / "strukcode"

    openapi_json = Path(args.out_spec) if args.out_spec else (sdk_dir / "openapi.json")
    build_dir = sdk_dir / ".build"
    out_pkg_dir = sdk_dir / "src" / "strukcode_ai"
    ...
    if args.source == "server":
        ...
        try:
            proc = run(["bun", "dev", "generate"], cwd=strukcode_dir)
    ...
    # 3) Locate generated module directory and copy to src/strukcode_ai
    ...
    # Patch __init__ to export StrukCodeClient if present
