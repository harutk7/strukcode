...
import { and, Database, eq, isNull, lt, or, sql } from "@strukcode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@strukcode-ai/console-core/schema/key.sql.js"
import { BillingTable, UsageTable } from "@strukcode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@strukcode-ai/console-core/util/price.js"
import { Identifier } from "@strukcode-ai/console-core/identifier.js"
import { Billing } from "@strukcode-ai/console-core/billing.js"
import { Actor } from "@strukcode-ai/console-core/actor.js"
import { WorkspaceTable } from "@strukcode-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@strukcode-ai/console-core/model.js"
import { BlackData } from "@strukcode-ai/console-core/black.js"
import { UserTable } from "@strukcode-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@strukcode-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@strukcode-ai/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  const FREE_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // strukcode bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-strukcode-session") ?? ""
    const requestId = input.request.headers.get("x-strukcode-request") ?? ""
    const projectId = input.request.headers.get("x-strukcode-project") ?? ""
    const ocClient = input.request.headers.get("x-strukcode-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-strukcode-request")
          headers.delete("x-strukcode-session")
          headers.delete("x-strukcode-project")
          headers.delete("x-strukcode-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo) {
    ...
    if (!billing.paymentMethodID)
      throw new CreditsError(
        `No payment method. Add a payment method here: https://strukcode.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    if (billing.balance <= 0)
      throw new CreditsError(
        `Insufficient balance. Manage your billing here: https://strukcode.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new MonthlyLimitError(
        `Your workspace has reached its monthly spending limit of $${billing.monthlyLimit}. Manage your limits here: https://strukcode.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new UserLimitError(
        `You have reached your monthly spending limit of $${authInfo.user.monthlyLimit}. Manage your limits here: https://strukcode.ai/workspace/${authInfo.workspaceID}/members`,
      )
  }
