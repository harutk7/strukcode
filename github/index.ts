...
import { createStrukcodeClient } from "@strukcode-ai/sdk"
...
const { client, server } = createStrukcode()
...
try {
  ...
  await assertStrukcodeConnected()
...
function createStrukcode() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`strukcode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createStrukcodeClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/strukcode|\/oc)(?=$|\s)/)) {
...
async function assertStrukcodeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to strukcode server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.strukcode.ai" : "https://strukcode.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.strukcode.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("strukcode-github-action")
    response = await fetch("https://api.strukcode.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `strukcode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/strukcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[strukcode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
